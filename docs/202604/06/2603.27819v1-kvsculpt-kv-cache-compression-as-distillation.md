...
tags: ["query:llm"]
score: 7.0
evidence: 针对长文本大模型推理的KV缓存压缩技术
tldr: 针对长文本LLM推理中的KV缓存压力，本文提出KVSculpt。不同于传统的剔除或合并方法，KVSculpt将压缩视为蒸馏过程，在连续嵌入空间中优化一组非约束的KV对，以精确模拟原始注意力行为。通过L-BFGS优化Key和最小二乘法求解Value，并结合自适应预算分配策略，在保持推理效率的同时大幅降低了信息损失。
source: arxiv
...
motivation: 现有的KV缓存压缩方法受限于原始缓存条目，无法通过连续空间优化来最大化保留注意力机制的原始行为。
method: 将KV压缩建模为蒸馏问题，利用L-BFGS优化Key并以最小二乘法求解Value，同时引入基于先导运行的跨层及跨头自适应预算分配。
result: 在Qwen2.5模型上，KVSculpt相比Select+Fit方法将KL散度降低了3.5-4.1倍，且自适应分配策略带来了额外的性能提升。
conclusion: 通过在连续空间优化KV对并实施细粒度的资源分配，可以显著提升KV缓存压缩的精度，证明了压缩难度在模型内部具有高度非均匀性。
---

## 摘要
KV 缓存压缩对于高效的长上下文大语言模型（LLM）推理至关重要。减少单对占用空间的方法（如量化和低秩分解）与减少缓存序列长度的方法是正交的。在序列长度维度上，现有方法涵盖了从纯剔除（选择保留哪些 KV 对）到合并（将相似的对组合成更少的对）的范围。两者都受限于原始缓存条目。我们提出了 KVSculpt，它走向了这一频谱的另一端：我们不再选择或组合原始对，而是在连续嵌入空间中优化一组更小的、不受约束的 KV 对，以保留每一层的注意力行为。Key 通过 L-BFGS 进行优化，Value 则通过最小二乘法以闭式解求解，每隔几步交替进行。在此基础上，我们引入了自适应预算分配，利用低成本的试点压缩运行，根据每个组件的难度在各层和 KV 头之间重新分配压缩预算。在具有 2048 token 上下文的 Qwen2.5-1.5B-Instruct 上，在压缩率 r ∈ {0.3, 0.5, 0.7} 的情况下，与 Select+Fit（基于注意力分数的剔除结合最小二乘 Value 拟合）相比，KVSculpt 将 KL 散度降低了 3.5-4.1 倍。自适应分配在不增加额外推理成本的情况下，使 KL 散度进一步降低了 1.3 倍。分析表明，压缩难度高度不均匀：各层之间的试点均方误差（MSE）差异高达 100 倍，而单层内的两个 KV 头之间的差异可达 467 倍，这证明了细粒度的预算分配至关重要。
