...
title_zh: 具有策略型代理的分布式随机梯度下降中的梯度操纵：具有收敛保证的真实性激励
authors: "Ziqin Chen, Yongqiang Wang"
date: 2026-03-30
pdf: "https://arxiv.org/pdf/2603.27962v1"
tags: ["query:marl"]
score: 6.0
evidence: 分布式学习中的策略性智能体与真实激励机制
tldr: 针对分布式学习中策略性代理可能操纵梯度以获取私利的问题，本文提出了一种完全分布式的支付机制。该机制首次在无需中心化服务器的情况下，同时实现了诚实行为保证和精确的收敛性能。通过理论证明，该方法在凸和强凸条件下均具有良好的收敛率，且能限制代理通过策略行为获得的累积收益，为分布式随机梯度下降提供了安全且高效的激励框架。
source: arxiv
...
result: 理论上证明了该机制在一般凸和强凸条件下的收敛性，并保证了策略性行为的累积收益有限，实验验证了其在标准机器学习任务上的有效性。
conclusion: 该研究成功解决了分布式学习中去中心化激励与收敛精度并存的难题，为构建鲁棒的协作学习系统提供了重要的理论支撑。
---

## 摘要
分布式学习因其在可扩展性、隐私保护和容错性方面的优势而受到广泛关注。在这种范式下，多个代理通过仅与其邻居交换参数来协作训练全局模型。然而，现有分布式学习方法的一个关键漏洞是它们隐含地假设所有代理在梯度更新过程中表现诚实。在现实场景中，这一假设往往失效，因为自私或策略型代理可能会为了个人利益而受到激励去操纵梯度，最终损害最终的学习结果。在这项工作中，我们提出了一种全分布式支付机制，首次在分布式随机梯度下降中同时保证了真实行为和准确收敛。这代表了一项重大进展，因为它克服了现有协作学习真实性机制的两个主要局限性：（1）依赖中心化服务器进行支付收集；（2）为了保证真实性而牺牲收敛精度。除了刻画一般凸和强凸条件下的收敛速度外，我们还证明了我们的方法能够保证代理通过策略行为获得的累积收益保持有限，即使迭代次数趋于无穷大——这是大多数现有真实性机制无法实现的特性。我们在基准数据集上对标准机器学习任务进行的实验结果证实了所提方法的有效性。
